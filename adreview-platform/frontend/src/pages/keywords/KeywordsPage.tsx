...
export default function KeywordsPage() {
  ...
  const openCreate = () => {
    ...
    createForm.setFieldsValue({ durationMode: 'permanent', kind: filterKind })
    setCreateOpen(true)
  }
    ...
    <div style={{ width: '100%' }}>
      ...
      >
        ...
        <Text type="secondary" style={{ fontSize: 13 }}>
          在策略配置时选择对应{filterKind}词库，对词库内的词条进行匹配
        </Text>
      </div>
      ...
      >
        <Space>
          <Tooltip
            title={
              effectiveOnly
                ? '仅显示当前在有效时间区间内且已启用的词库'
                : '显示所有词库，包括已过期 / 即将生效 / 已停用'
            }
          >
            <Radio.Group
      ...
      <Table<LibraryListItem>
        ...
        size="middle"
        scroll={{ x: true }}
        locale={{ emptyText: `当前筛选条件下暂无${filterKind}词库,点击右上角新建` }}
      />

      <Drawer
        open={createOpen}
        onClose={() => setCreateOpen(false)}
        title={`新建${filterKind}词库`}
        width={560}
        extra={
          <Space>
      ...
      >
        <Form<CreateFormValues>
          form={createForm}
          layout="vertical"
          initialValues={{ durationMode: 'permanent', kind: filterKind }}
        >
          <Form.Item
            name="kind"
            label="类型"
