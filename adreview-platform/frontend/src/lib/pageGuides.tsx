  ...
  sections: [
    {
      ...
      markdown:
        '1. **新增模型页面**有一个「接入测试」按钮,点击测试时,返回当前模型服务的模型标签。\n'
        + '2. **后端预置接入校验的逻辑**:包括默认的文本 / 图片测试数据,测试服务并返回模型标签列表。\n'
        + '3. 接入校验没问题后,**保存新增模型**时,模型详情页的「模型标签」自动填入检测的模型标签。\n'
        + '4. **基于模型标签匹配业务标签**:已经匹配的标签不能再次被选择。\n'
        + '5. **测试模型的返回接口**要返回已经映射好的业务标签(后端做模型标签 → 业务标签的映射)。\n'
        + '6. **标签管理页面只负责启用和新增业务标签** —— 不参与模型与标签的绑定 / 解绑。\n\n'
        + '> 一句话:小模型发现能识别哪些业务标签,业务标签只声明自己存在,二者映射关系由后端模型测试接口维护。',
    },
    {
      heading: '页面职责(职责单一)',
      markdown:
        '**标签管理页面 = 业务标签的 CRUD 与启用控制**,不参与模型绑定:\n\n'
        + '- ✅ 新增一级 / 二级 / 三级业务标签\n'
        + '- ✅ 编辑名称、状态、是否启用、可适用模态\n'
        + '- ✅ 启用 / 停用业务标签(启用前需通过引用检查)\n'
        + '- ✅ 删除业务标签(有引用时阻止并弹窗)\n'
        + '- ❌ 不绑定 / 解绑小模型\n'
        + '- ❌ 不读取 / 修改模型返回的模型标签\n'
        + '- ❌ 不维护模型标签 → 业务标签的映射关系\n\n'
        + '模型绑定操作在「模型管理 → 小模型」详情页通过模型测试驱动。',
    },
    {
      heading: '页面操作流程',
      markdown:
        ...
        + '2. 切到「三级」 → 选父级一级 → 选父级二级\n'
        + '3. 输入「三级标签名称」\n'
        + '4. 选择「适用模态」(可多选;每个模态生成一行独立记录)\n'
        + '5. 选「状态」(启用 / 停用) + 「可启用」(enabled)\n'
        + '6. 保存 → 列表按所选模态数量生成 N 条记录\n\n'
        + '**启用 / 停用**\n\n'
        + '1. 点击列表行「状态」chip 切换 active ↔ inactive\n'
        + '2. 若被 active 审核策略引用,弹「引用清单」阻止\n'
        + '3. 无引用则直接切换\n\n'
        + '**删除**\n\n'
        + '1. 点击「删除」 → 后端检查引用\n'
        + '2. 有任何引用(策略 / 模型)→ 弹「引用清单」阻止\n'
        + '3. 无引用 → 级联删除(自身 + 子标签)',
    },
    {
      heading: '数据口径',
      markdown:
        '**三级标签多模态拆行**\n\n'
        + '- 创建时选 N 个模态 → 列表生成 N 条独立记录\n'
        + '- 每条记录共享 name + parentId + status + enabled,各自有独立 modality\n'
        + '- 编辑时只改当前单条;兄弟记录独立(Q2 不合并)\n\n'
        + '**enabled 与模型绑定**\n\n'
        + '- `enabled` = 是否可被审核策略引用\n'
        + '- 三级标签未绑定模型时,`enabled = false` (列表显示「未启用(缺模型)」)\n'
        + '- 模型绑定后(其他模块操作) → `enabled` 可切换为 true\n'
        + '- 编辑模式下 enabled 字段强制 disabled,避免误改\n\n'
        + '**引用清单**(停用 / 删除前)\n\n'
        + '- 后端 GET `/api/v1/tags/{id}/references` 返回 strategies + models\n'
        + '- `can_deactivate = 没有 active 策略引用`\n'
        + '- `can_delete = 任何引用都没有`\n'
        + '- 阻止时返回 409 + references 详情,前端弹 Modal',
    },
  ],
}
