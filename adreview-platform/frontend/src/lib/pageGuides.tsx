...
const reportsGuide: PageGuide = {
  ...
  sections: [
    ...
    },
    {
      ...
      markdown:
        '规则表(降级为列表,保持原列顺序):\n\n'
        + '- **规则 1 · 拒绝率异常** | **指标** 拒绝率 | **时间窗口** 近 1 小时 | **维度** 审核模态 | **算法** 固定阈值 | **严重程度** > 5% 严重;> 3% 提醒 | **状态** 启用\n'
        + '- **规则 2 · 账号高风险阻断异常** | **指标** 高风险阻断密度 | **时间窗口** 近 1 小时 | **维度** 全局 | **算法** 固定阈值 | **严重程度** > 30% 严重;> 20% 提醒;**且** count > 20 | **状态** 启用\n'
        + '- **规则 3 · 高风险账号聚集异常** | **指标** 高风险账号密度 | **时间窗口** 近 1 小时 | **维度** 全局 | **算法** 固定阈值 | **严重程度** > 50% 严重;> 30% 提醒;**且** account count > 5 | **状态** 启用',
    },
  ],
}
