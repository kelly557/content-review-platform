    ...
    <div style={{ width: '100%' }}>
      ...
      >
        <Space size={12} align="center" wrap>
          ...
          {/* 存量 reply 库可能保留了 risk_point,这里只读展示(无编辑入口) */}
          {library?.risk_point && (
            <Tooltip
              title={`存量代答库:使用位置 ${library.risk_point.item_name ?? ''} / ${
                library.risk_point.label_cn || library.risk_point.label
              }`}
            >
              <Tag color="default" style={{ margin: 0 }}>
                使用位置·{library.risk_point.item_name ?? '—'} / {library.risk_point.label_cn || library.risk_point.label}
              </Tag>
            </Tooltip>
          )}
        </Space>
